...
class GarmentsAutomationApp:
    ...
    def run_process(self):
        ...
        try:
            ...
            wb = openpyxl.load_workbook(mas_file)
            self.fill_total_summary_bottom(wb)
            wb.save(mas_file)
    ...
    def update_master_file(self, filepath, data):
        ...
        for entry in data:
            ...
            try:
                ...
                # V44 FIX: Pass 'ws' explicitly to update summary
    ...
    # ==========================================
    # V35: DATE WISE SUMMARY ENGINE (V43 FIX)
    # ==========================================
    ...
    def fill_total_summary_bottom(self, wb):
        ...
        r = start_row
        for style_key, data in STYLE_WISE_TOTALS.items():
            ws.cell(r, 1).value = data["buyer"]
            ws.cell(r, 2).value = data["style"]
            ws.cell(r, 3).value = data["gg"]
            ws.cell(r, 4).value = data["order"]
            ws.cell(r, 5).value = data["link"]
            ws.cell(r, 6).value = (data["order"] or 0) - (data["link"] or 0)
            ws.cell(r, 7).value = data["smv"]
            ws.cell(r, 8).value = data["output"]
            ws.cell(r, 9).value = data["wmin"]
            ws.cell(r, 10).value = data["output"] * data["smv"]
            ws.cell(r, 11).value = len(data["days"])

            eff = (data["output"] * data["smv"]) / data["wmin"] if data["wmin"] else 0
            ws.cell(r, 12).value = eff
    ...
    # ==============================
# PHASE 6 – GHOST DATE FIX + TOTAL SUMMARY EXTENSION
# ==============================

# ONLY NEW GLOBAL STORAGE
VALID_SUPERVISOR_DATES = set()
STYLE_WISE_TOTALS = {}

# --------------------------------
# PATCH 1: Supervisor Reader FIX
# --------------------------------
def read_supervisor_file(self, filepath):
    wb = openpyxl.load_workbook(filepath, data_only=True)
    all_data = []
    global VALID_SUPERVISOR_DATES, STYLE_WISE_TOTALS
    VALID_SUPERVISOR_DATES.clear()
    STYLE_WISE_TOTALS.clear()
    ...
    for sheet in wb.worksheets:
        ...
        for r_idx, row in enumerate(rows):
            found_date = self.find_date_in_row(row)
            if found_date:
                row_date_map[r_idx] = found_date
                VALID_SUPERVISOR_DATES.add(found_date.date())
        ...
        for r_idx, row in enumerate(rows):
            ...
            if "styleno" in row_strict and "today" in row_strict:
                ...
                for i in range(r_idx, -1, -1):
                    if i in row_date_map:
                        closest_date = row_date_map[i]
                        break
                if not closest_date:
                    continue

                col = {}
                for idx, v in enumerate(row_loose):
                    if "style" in v: col["style"] = idx
                    elif "today" in v: col["today"] = idx
                    elif "buyer" in v: col["buyer"] = idx
                    elif "gg" == row_strict[idx]: col["gg"] = idx
                    elif "order" in v and "qty" in v: col["order"] = idx
                    elif "con" in v and "qty" in v: col["link"] = idx
                    elif "smv" in v: col["smv"] = idx
                    elif "min" in v: col["min"] = idx
                ...
                while j < len(rows):
                    ...
                    try:
                        style = str(d[col["style"]]).strip()
                        out = float(d[col["today"]])
                        if out <= 0:
                            continue

                        VALID_SUPERVISOR_DATES.add(closest_date.date())

                        entry = {
                            "date": closest_date,
                            "style": style,
                            "output": out,
                            "buyer": d[col.get("buyer")],
                            "gg": d[col.get("gg")],
                            "order": d[col.get("order")],
                            "link": d[col.get("link")],
                            "smv": float(d[col.get("smv", 0)] or 0),
                            "min": float(d[col.get("min", 0)] or 0)
                        }
                        all_data.append(entry)

                        # ---- TOTAL SUMMARY ACCUMULATION ----
                        s = style.lower()
                        if s not in STYLE_WISE_TOTALS:
                            STYLE_WISE_TOTALS[s] = {
                                "buyer": entry["buyer"],
                                "style": style,
                                "gg": entry["gg"],
                        ...
                        STYLE_WISE_TOTALS[s]["output"] += out
                        STYLE_WISE_TOTALS[s]["wmin"] += entry["min"]
                        STYLE_WISE_TOTALS[s]["days"].add(closest_date.date())
...
def read_supervisor_file(self, filepath):
    ...
    # --------------------------------
# PATCH 2: TOTAL SUMMARY BOTTOM TABLE
# --------------------------------
def fill_total_summary_bottom(self, wb):
    if "TOTAL SUMMARY" not in wb.sheetnames:
        return
    ...
    for style_key, data in STYLE_WISE_TOTALS.items():
        ...
        ws.cell(r, 5).value = data["link"]
        ws.cell(r, 6).value = (data["order"] or 0) - (data["link"] or 0)
        ws.cell(r, 7).value = smv
        ws.cell(r, 8).value = output
        ws.cell(r, 9).value = wmin
        ws.cell(r, 10).value = pmin
        ws.cell(r, 11).value = len(data["days"])
        ws.cell(r, 12).value = eff
        ws.cell(r, 12).number_format = "0%"
