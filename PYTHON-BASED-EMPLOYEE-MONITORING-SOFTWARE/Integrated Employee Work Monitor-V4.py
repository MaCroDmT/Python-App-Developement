...
class UnifiedMonitorApp:
    ...
    def capture_instant_screenshot(self):
        try:
            with mss.mss() as sct:
                now = datetime.now()
            ...
            logging.error(f"Instant screenshot failed: {e}")
    ...
    def clock_in(self):
        with self.state_lock:
            ...
            if self.on_break:
                ...
                self.capture_instant_screenshot()
    ...
    def monitor_activity_loop(self):
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if not self.check_internet(): pass
                self.stop_event.wait(1)
        except Exception as e:
    ...
    def update_live_timer(self):
        ...
        last_added_work_duration = 0

        for entry in self.log_data:
            start_str = entry['Start Time']
            raw_duration = entry['Activity Duration']
            duration = self.parse_duration(raw_duration).total_seconds()
            
            if "Worked for" in raw_duration:
                if start_str == last_work_start:
                    diff = duration - last_added_work_duration
                    total_work_sec += diff
                else:
                    total_work_sec += duration
                last_work_start = start_str
                last_added_work_duration = duration
                
            elif "Took a break" in raw_duration:
                total_break_sec += duration

        if self.is_working and not self.on_break:
            current_session = (datetime.now() - self.start_time).total_seconds()
            if self.start_time and self.start_time.strftime("%I:%M %p") == last_work_start:
                 total_work_sec += (current_session - last_added_work_duration)
            else:
                 total_work_sec += current_session
    ...
    def analyze_data(self, df):
        ...
        for index, row in df.iterrows():
            raw = row['Activity Duration']
            start_str = row['Start Time']
            duration = self.parse_duration(raw).total_seconds()
            
            if "Worked for" in raw:
                if start_str == last_work_start:
                    work_sec += (duration - last_added_work_duration)
                else:
                    work_sec += duration
                last_work_start = start_str
                last_added_work_duration = duration
                
            elif "Took a break" in raw:
                break_sec += duration
            elif "System locked" in raw:
                locked_sec += duration
            elif "Internet interrupted" in raw:
                net_sec += duration
        
        total_idle_sec = break_sec + locked_sec + net_sec
        total_session_time = work_sec + total_idle_sec
        if total_session_time > 0: idle_percentage = (total_idle_sec / total_session_time) * 100
        else: idle_percentage = 0.0
    ...
    def save_summary_to_excel(self):
        ...
        try:
            ...
            summary_data = {
                ...
                'Value': [analysis['Worked for in a day'], analysis['System locked/clocked out'], analysis['Took a break'], analysis['Internet Interrupted'], analysis['Idle Percentage']]
            }
    ...
    def create_unified_report_stable(self):
        ...
        for entry in self.log_data:
            ...
            if " for " in raw:
                ...
                curr_sec = self.parse_duration(raw).total_seconds()
                
                if "Worked" in activity:
                    if start_str == last_work_start:
                        total_work_seconds += (curr_sec - last_added_work_duration)
                    else:
                        total_work_seconds += curr_sec
                    
                    last_work_start = start_str
                    last_added_work_duration = curr_sec
                    
                elif "Took a break" in activity:
                    total_break_seconds += curr_sec
            
            table_data.append([start_str, end, activity, duration])
